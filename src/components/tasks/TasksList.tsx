...
export const TasksList: React.FC = () => {
  ...
  const reorderTasks = useCallback(
    (tasks: Task[]): Task[] => {
      ...
      const dateFilterFunc = (task: Task) => {
        switch (dateFilter) {
          case "today":
            return isToday(new Date(task.date)) || 
                   (task.deadline && isToday(new Date(task.deadline)));
          case "this-week":
            return isThisWeek(new Date(task.date)) || 
                   (task.deadline && isThisWeek(new Date(task.deadline)));
          case "custom":
            return isInDateRange(new Date(task.date), customDateRange) ||
                   (task.deadline && isInDateRange(new Date(task.deadline), customDateRange));
          default:
            return true;
        }
      };
      
      unpinnedTasks = unpinnedTasks.filter(dateFilterFunc);
      pinnedTasks = pinnedTasks.filter(dateFilterFunc);
